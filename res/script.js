...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Initial Projekt-Elemente sortieren
    changeSort();
});

function checkCheck(className) {
    //Button aktivieren
    el_id = className + "_button";
...
function displayList() {
    var cards = document.getElementsByClassName('Navigation')[0].children;
    ...
    if (arr.length > 0) {
        //Ab dem ersten positiven Match wird Card sichtbar
        for (var j = arr.length - 1; j >= 0; j--) {
            className = arr[j]
            for (var i = cards.length - 1; i >= 0; i--) {
                var tags = cards[i].dataset.tags;
        ...
        //Wenn kein Filter ausgewählt ist, alle Cards anzeigen
        for (var i = cards.length - 1; i >= 0; i--) {
            cards[i].classList.remove("hidden");
        }
    }
...
function setFooter() {
    ...
    <a href="https://www.linkedin.com/in/thilo-hohl-4602793a2/" class="fa fa-linkedin" target="_blank"></a>
...
function changeSort() {
    ...
    document.getElementById("sort_button_icon").classList.toggle("rotated");

    var parent = document.getElementsByClassName('Navigation')[0],
        divs = parent.children,
        i = divs.length - 1;

    for (; i--;) { parent.appendChild(divs[i]) }
}